...
class GVPTransformerModel(nn.Module):
    ...
    def sample(self, coords, partial_seq=None, temperature=1.0, confidence=None, device=None):
        ...
        # print(f"batch_converter.alphabet.cls_idx: {batch_converter.alphabet.cls_idx}")


        # batch coords with first and last tensor on the dim 1 to be inf as padding
    ...
    def sample_batch(self, batch_coords, confidence, padding_mask, partial_seq=None, device=None):
        """
        Samples sequences based on multinomial sampling (no beam search).
        """
